...
// DataView2DModelGtk.cs: Gtk.ITreeModel for UI.
...
namespace ScoobyRom
{
	...
	public sealed class DataView2DModelGtk : DataViewModelBaseGtk
	{
		...
		public void SetNodeContent (TreeIter iter, Table2D table2D)
		{
			...
			store.SetValue (iter, (int)ColumnNr2D.Location, table2D.LocationCorrected(d));
			store.SetValue (iter, (int)ColumnNr2D.XPos, table2D.RangeX.PosCorrected(d));
			store.SetValue (iter, (int)ColumnNr2D.YPos, table2D.RangeY.PosCorrected(d));

...
* Windows x64 example: `C:\Program Files\ScoobyRom`
...
**.NET 8.0** and **Gtk#** at least, see chapter 4.

#### Architecture

ScoobyRom is compiled as 64 bit application. **32 bit is not supported.**
This applies only to the ScoobyRom (GUI app) itself. Extra plot windows are generated by
automatically launching independent external tool _gnuplot_, therefore *gnuplot* x64 is working fine and recommended.
...
Version: **.NET v8.0 or higher**

*	Windows 10

	* Minimal version is 1607

*	Windows 11

	* Minimal version is 22H2
...
*	**Linux/Mac**:
	...
	* Not supported because System.Drawing package package
	is only supported on Windows operating systems In .NET 6 and later versions.


### <a name="gtksharp"></a> 4.2) Gtk3 runtime

REQUIRED!

The application will not run without this as it is used to show its (multi-platform) *graphical user interface* (GUI).

[Install GTK from MSYS2 packages](https://www.gtk.org/docs/installations/windows/)

[GTK for Windows Runtime Environment Installer: 64-bit](https://github.com/tschoonj/GTK-for-Windows-Runtime-Environment-Installer)
...
Launch the binary (Windows: `gnuplot.exe`)
...
Newline type does not matter, as *ScoobyRom* and *gnuplot* can read both. Therefore you can easily share such files across operating systems.
...
### <a name="config"></a> 5.1) ScoobyRom.dll.config
...
The appropriate path for gnuplot is important!
...
Windows 10 x64 + gnuplot 5.0.1 x64 (`gp501-win64-mingw.exe`) using default install path (`C:\Program Files\gnuplot`)
...
#### Debug console
Application writes some diagnostic messages to a console window which is hidden by default. The following parameter enables debug console if set to `true`.

	<add key="openConsoleWindow" value="false"/>
...
For MPC5746 support RomRaider v1.0.1 or newer is required.
